...
import { PackageJSON, Release, VersionType } from "@changesets/types";
...
export default async function createChangeset(
  ...
  if (allPackages.length > 1) {
    if (bump) {
      releases.push({ name: changedPackages[0], type: bump as VersionType });
    } else {
      const packagesToRelease = !bump
  ...
  let summary = summaryText || (await cli.askQuestion("Summary"));
  if (summary.length === 0) {
    try {
